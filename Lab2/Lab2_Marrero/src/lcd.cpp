...
void fourBitCommandWithDelay(unsigned char data, unsigned int delay){
  ...
  PORTB |= (1 << PORTB4); //setting enable btipin high
  delayUs(1);
  PORTB &= ~(1 << PORTB4); //setting enable bit pin low
  delayUs(delay);
}
...
void writeCharacter(unsigned char character){
  ...
  PORTB |= (1 << PORTB6); //setting rs bit high
  PORTB |= (1 << PORTB4); //setting enable bit  pin high
  delayUs(1);
  PORTB &= ~(1 << PORTB4); //setting enable bit pin low
  //delayUs(1);
  PORTA = (PORTA & 0xF0) | (character & 0x0F); //bottom most bits of command snet
  PORTB |= (1 << PORTB4); //setting enable bit pin high
  delayUs(1);
  PORTB &= ~(1 << PORTB4); //setting enable bit pin low
  delayUs(46);
...
void initLCDProcedure(){
  ...
  eightBitCommandWithDelay((unsigned char)(0x06), 100);
  ...
  eightBitCommandWithDelay((unsigned char)(0x0C), 53);
